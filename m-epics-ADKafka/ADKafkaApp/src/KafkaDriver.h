...
class epicsShareClass KafkaDriver : public ADDriver {
  public:
     ...
     * Configures the driver and starts the consumer thread. Note that consumption of messages is
     * not started but must be done seperately using the appropriate PV.
     * @param[in] portName Name of the asyn port driver to be created. Can be used to connect a
     * plugin to that particular detector.
     * @param[in] maxBuffers The maximum number of NDArray buffers that the NDArrayPool used by the
     * driver will allocate. There will be no limitation if this is set to 0 though this is not
     * recommended a bug in the driver or a plugin can make the driver consume all the available
     * RAM.
     * @param[in] maxMemory The maximum amount of memory (in bytes) that the NDArrayPool will
     * allocate for data in all the NDArray objects in the pool. Setting this to 0 will allow an
     * unlimited amount of data. Can be used instead of or in conjunction with the previous
     ...
     * @param[in] brokerTopic Topic from which the driver should consume messages. Note that only
     ...
     * should be running. It then waits for an event from the thread which is created when it
     * it actually exits.
     * @note If the destructor is called before the thread has actually started the event will never
     * be created which will cause a deadlock. This problem should only be visible during unit
     * testing.
     * @todo Fix the constructor and destructor to actually keep track of the state of the thread
  ...
  protected:
    ...
    /** @brief Used to pass an event from the processing thread to the thread calling the destructor
