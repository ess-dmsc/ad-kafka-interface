 ...
 * @param[in] pNDArrayPool A pointer to the NDArrayPool which is used to allocate NDArray which
 * will store the data in the buffer.
 * @param[in] bufferPtr Pointer to the buffer containing the data which is to be deserialized.
 * @param[in] size Size of the data in bytes. Note that this value is not used and that no checks
 * are made for overflows.
 * @param[out] pArray The pointer to the NDArray containing the deserialized data. Note that the
