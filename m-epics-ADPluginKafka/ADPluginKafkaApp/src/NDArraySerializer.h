 ...
 * The C++ flatbuffers implementatione has an internal buffer for storing the serialized data. Thus
 * it only stores the data for one serialized NDArray. If more are required, copy the data or use
 * several instances of this class.
 */
class NDArraySerializer {
  public:
    /** @brief Initialize the flatbuffer builder with a given buffer size.
     * The default buffer size given here is 1MB though. If the buffer is to small to store the
     * serialized data it will be automatically expanded to an appropriate size. In order to
     * minimise the number of deallocations and allocations of memory, provide an buffer size.
     * @param[in] bufferSize Size of flatbuffer buffer in bytes. Will be increase if the data does
     * not fit.
     */
     ...
     * @param[out] bufferPtr The pointer to the serialized data. Note that this pointer is only
  ...
  protected:
     ...
     * This is used by the algorithm that packs attributes into the flatbuffer. Could be a function
     * only available in the implementation file but is used by the unit tests and thus has to be
     * made available here.
