 ...
 * straight from the documentation of NDPluginDriver::NDPluginDriver().
     ...
     * @param[in] queueSize The number of NDArrays that the input queue for this plugin can hold
 * when
     *            NDPluginDriverBlockingCallbacks=0.  Larger queues can decrease the number of
 * dropped arrays,
     *            at the expense of more NDArray buffers being allocated from the underlying
 * driver's NDArrayPool.
     * @param[in] blockingCallbacks Initial setting for the NDPluginDriverBlockingCallbacks flag.
     *            0=callbacks are queued and executed by the callback thread; 1 callbacks execute in
 * the thread
     *            of the driver doing the callbacks.
     * @param[in] NDArrayPort Name of asyn port driver for initial source of NDArray callbacks.
     * @param[in] NDArrayAddr asyn port driver address for initial source of NDArray callbacks.
     * @param[in] maxAddr The maximum  number of asyn addr addresses this driver supports. 1 is
 * minimum.
     * @param[in] maxBuffers The maximum number of NDArray buffers that the NDArrayPool for this
 * driver is
     *            allowed to allocate. Set this to -1 to allow an unlimited number of buffers.
     * @param[in] maxMemory The maximum amount of memory that the NDArrayPool for this driver is
     *            allowed to allocate. Set this to -1 to allow an unlimited amount of memory.
     * @param[in] interfaceMask Bit mask defining the asyn interfaces that this driver supports.
     * @param[in] interruptMask Bit mask definining the asyn interfaces that can generate interrupts
 * (callbacks)
     * @param[in] asynFlags Flags when creating the asyn port driver; includes ASYN_CANBLOCK and
 * ASYN_MULTIDEVICE.
