...
void NDArraySerializer::SerializeData(NDArray &pArray, unsigned char *&bufferPtr, size_t &bufferSize) {
    ...
    auto epics_ts =
    FB_Tables::epicsTimeStamp(pArray.epicsTS.secPastEpoch, pArray.epicsTS.nsec);
    ...
    auto payload = builder.CreateVector((unsigned char *)pArray.pData, ndInfo.totalBytes);
    ...
    while (attr_ptr != NULL) {
        ...
        if (ND_SUCCESS == attrValueRes) {
            ...
            builder.CreateVector((unsigned char *)attrValueBuffer, bytes);
            
            auto attr =
            FB_Tables::CreateNDAttribute(builder, temp_attr_str, temp_attr_desc,
...
void NDArraySerializer::ReleaseSerializedData() {
